...
class OptimizerTcpManager:
    ...
    def __init__(self, optimizer_path=None, ip=None, port=None, debug=False):
        """
        ...
        Args:
            ...
            :param debug: print log statements
        Returns:
    ...
    def __send_receive_data(self, text_to_send, buffer_size=512, max_data_size=1048576):
        ...
        conn_socket.send(str(len(encoded_data)).zfill(8).encode())
        #self.conn_socket.sendall(encoded_data)
        ...
        for _i in range(max_read_rounds):
            #data_chunk = self.conn_socket.recv(buffer_size)
    ...
    def start(self):
        ...
        def threaded_start():
            ...
            logging.info("Starting TCP/IP server at %s:%d (in a detached thread)",
                         ip, port)
            if self.__debug:
                command = ['powershell.exe', '$env:RUST_LOG="info";', 'cargo', 'run', '-q']
            else:
                command = ['cargo', 'run', '-q']
        ...
        time.sleep(0.1)
        #self.conn_socket = self.__obtain_socket_connection()
        self.ping()
