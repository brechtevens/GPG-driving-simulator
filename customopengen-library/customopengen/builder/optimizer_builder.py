...
import customopengen.config as og_cfg
import customopengen.definitions as og_dfn
import customopengen.constraints as og_con
...
_AUTOGEN_RECTANGLE_LOWER_BOUNDS_FNAME = 'auto_casadi_rectangle_lower.c'
_AUTOGEN_RECTANGLE_UPPER_BOUNDS_FNAME = 'auto_casadi_rectangle_upper.c'
...
class OpEnOptimizerBuilder:
    ...
    def __generate_memory_code(self, cost=None, grad=None, f1=None, f2=None, lbounds=None, ubounds=None):
        """
        ...
        :param lbounds: mapping Rectangle lower bounds (cs.Function)
        :param ubounds: mapping Rectangle upper bounds  (cs.Function)
        """
    ...
    def __generate_mapping_Rectangle_bounds(self) -> cs.Function: #Selfmade code
        self.__logger.info("Defining function returning the Rectangle bounds of U(p)")
        problem = self.__problem
        p = problem.parameter_variables
        constraints = problem.constraints

        if isinstance(constraints, og_con.Rectangle):
            lbounds = [-float('inf')] if constraints.xmin is None else constraints.xmin
            ubounds = [float('inf')] if constraints.xmax is None else constraints.xmax
        else:
            lbounds = []
            ubounds = []

        mapping_Rectangle_lower_fun = cs.Function(
            "rectangle_lower_" + self.__build_config.id,
            [p], [cs.vertcat(*lbounds)])

        mapping_Rectangle_upper_fun = cs.Function(
            "rectangle_upper_" + self.__build_config.id,
            [p], [cs.vertcat(*ubounds)])
        return mapping_Rectangle_lower_fun, mapping_Rectangle_upper_fun
    ...
    def __generate_casadi_code(self):
        ...
        # -----------------------------------------------------------------------
        mapping_lower_fun, mapping_upper_fun = self.__generate_mapping_Rectangle_bounds() #Selfmade
        lower_file_name = "rectangle_lower_" + self.__build_config.id + ".c"
        upper_file_name = "rectangle_upper_" + self.__build_config.id + ".c"
        mapping_lower_fun.generate(lower_file_name)
        mapping_upper_fun.generate(upper_file_name)

        icasadi_extern_dir = os.path.join(self.__icasadi_target_dir(), "extern")
        shutil.move(lower_file_name, os.path.join(icasadi_extern_dir, _AUTOGEN_RECTANGLE_LOWER_BOUNDS_FNAME))
        shutil.move(upper_file_name, os.path.join(icasadi_extern_dir, _AUTOGEN_RECTANGLE_UPPER_BOUNDS_FNAME))

        # -----------------------------------------------------------------------
        self.__generate_memory_code(psi_fun, grad_psi_fun,
                                    mapping_f1_fun, mapping_f2_fun,
                                    mapping_lower_fun, mapping_upper_fun)
    ...
    def __generate_yaml_data_file(self):
        ...
        build_details = {'id': build_config.id,
                         ...
                         'rectangle_lower': "rectangle_lower_" + self.__build_config.id,
                         'rectangle_upper': "rectangle_upper_" + self.__build_config.id
                         }
