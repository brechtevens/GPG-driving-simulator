...
/*
 * CasADi interface for the rectangle lower bounds
 */
extern int rectangle_lower_{{build_config.id}}(
    const casadi_real** arg, 
    casadi_real** res, 
    casadi_int* iw, 
    casadi_real* w, 
...
/*
 * CasADi interface for the rectangle upper bounds
 */
extern int rectangle_upper_{{build_config.id}}(
    const casadi_real** arg, 
    casadi_real** res, 
    casadi_int* iw, 
    casadi_real* w, 
...
#if LB_SZ_IW_{{ meta.optimizer_name | upper}} > 0
static casadi_int allocated_i_workspace_lb[LB_SZ_IW_{{ meta.optimizer_name | upper}}];      /* lb (int )    */
#else
static casadi_int *allocated_i_workspace_lb = NULL;
#endif

#if UB_SZ_IW_{{ meta.optimizer_name | upper}} > 0
static casadi_int allocated_i_workspace_ub[UB_SZ_IW_{{ meta.optimizer_name | upper}}];      /* ub (int )    */
#else
static casadi_int *allocated_i_workspace_ub = NULL;
#endif
...
#if LB_SZ_W_{{ meta.optimizer_name | upper}} > 0
static casadi_real allocated_r_workspace_lb[LB_SZ_W_{{ meta.optimizer_name | upper}}];      /* lb (real )   */
#else
static casadi_real *allocated_r_workspace_lb = NULL;
#endif

#if UB_SZ_W_{{ meta.optimizer_name | upper}} > 0
static casadi_real allocated_r_workspace_ub[UB_SZ_W_{{ meta.optimizer_name | upper}}];      /* ub (real )   */
#else
static casadi_real *allocated_r_workspace_ub = NULL;
#endif
...
#if LB_SZ_RES_{{ meta.optimizer_name | upper}} > 0
static casadi_real *result_space_lb[LB_SZ_RES_{{ meta.optimizer_name | upper}}];            /* lb (res )    */
#else
static casadi_real **result_space_lb = NULL;
#endif

#if UB_SZ_RES_{{ meta.optimizer_name | upper}} > 0
static casadi_real *result_space_ub[UB_SZ_RES_{{ meta.optimizer_name | upper}}];            /* ub (res )    */
#else
static casadi_real **result_space_ub = NULL;
#endif
...
/**
 * Copy (p) into uxip_space
 */
static void copy_args_into_p_space(const casadi_real** arg) {
    int i;
    for (i=0; i<NP_{{ meta.optimizer_name | upper}}; i++) uxip_space[NU_{{ meta.optimizer_name | upper}}+NXI_{{ meta.optimizer_name | upper}}+i] = arg[0][i];  /* copy p  */
}
...
int rectangle_lower_function_{{ meta.optimizer_name }}(const casadi_real** arg, casadi_real** res) {
    /* Array of pointers to where (p) is stored */
    const casadi_real* args__[LB_SZ_ARG_{{ meta.optimizer_name | upper}}] =
            {uxip_space + NU_{{ meta.optimizer_name | upper}} + NXI_{{ meta.optimizer_name | upper}}};  /* :p   */
    /* Copy given data to variable `uxip_space` */
    copy_args_into_p_space(arg);
    /*
     * The result should be written in result_space_lb
     * (memory has been allocated - see beginning of this file)
     */
    result_space_lb[0] = res[0];
    /*
     * Call auto-generated function Rectangle_lower_bounds_{{build_config.id}}
     * Implemented in: icasadi/extern/auto_casadi_rectangle_lower.c
     */
    return rectangle_lower_{{build_config.id}}(
        args__,
        result_space_lb,
        allocated_i_workspace_lb,
        allocated_r_workspace_lb,
        (void*) 0);
}

/* ------MAPPING UB------------------------------------------------------------- */

int rectangle_upper_function_{{ meta.optimizer_name }}(const casadi_real** arg, casadi_real** res) {
    /* Array of pointers to where (p) is stored */
    const casadi_real* args__[UB_SZ_ARG_{{ meta.optimizer_name | upper}}] =
            {uxip_space + NU_{{ meta.optimizer_name | upper}} + NXI_{{ meta.optimizer_name | upper}}};  /* :p   */
    /* Copy given data to variable `uxip_space` */
    copy_args_into_p_space(arg);
    /*
     * The result should be written in result_space_ub
     * (memory has been allocated - see beginning of this file)
     */
    result_space_ub[0] = res[0];
    /*
     * Call auto-generated function Rectangle_upper_bounds_{{build_config.id}}
     * Implemented in: icasadi/extern/auto_casadi_rectangle_upper.c
     */
    return rectangle_upper_{{build_config.id}}(
        args__,
        result_space_ub,
        allocated_i_workspace_ub,
        allocated_r_workspace_ub,
        (void*) 0);
}