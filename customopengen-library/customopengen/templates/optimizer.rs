...
pub fn solve(
    ...
    {% if 'Rectangle' == problem.constraints.__class__.__name__ -%}
    let mut lb :[f64; {{meta.optimizer_name|upper}}_NUM_DECISION_VARIABLES] = [0.0; {{meta.optimizer_name|upper}}_NUM_DECISION_VARIABLES];
    let mut ub :[f64; {{meta.optimizer_name|upper}}_NUM_DECISION_VARIABLES] = [0.0; {{meta.optimizer_name|upper}}_NUM_DECISION_VARIABLES];
    icasadi_{{meta.optimizer_name}}::rectangle_lower(&p, &mut lb);
    icasadi_{{meta.optimizer_name}}::rectangle_upper(&p, &mut ub);
    let bounds = Rectangle::new(Some(&lb), Some(&ub));
    {% if problem.dim_decision_variables() <= 32 %}
    info!("Lower bound decision variables: {:?}", lb);
    info!("Upper bound decision variables: {:?}", ub);
    {% endif %}
    {% else %}
    ...
    info!("AlmProblem succesfully constructed");
