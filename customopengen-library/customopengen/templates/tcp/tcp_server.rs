...
fn setup_listener(tcp_config: &TcpServerConfiguration) -> Result<std::net::TcpListener, std::io::Error> {
    let listener = TcpBuilder::new_v4()?
        .reuse_address(true)?
        .bind(format!("{}:{}", tcp_config.ip, tcp_config.port))?
        .listen(42)?;
...
fn run_server(tcp_config: &TcpServerConfiguration) {
    ...
    info!("Done");
    let listener = setup_listener(&tcp_config).unwrap();
    ...
    for stream in listener.incoming() {
        ...
        let mut length_buffer = vec![0u8; 8];
        let mut bytes_buffer = vec![0u8; READ_BUFFER_SIZE];
        let mut read_data_length;
        let mut buffer = String::new();
        let mut bytes_to_read: usize = 1;
        while bytes_to_read != 0 {
            stream.read(&mut length_buffer).expect("count not read stream");
            let new_string = String::from_utf8(length_buffer.to_vec())
                .expect("sent data is not UFT-8");
            bytes_to_read = new_string.parse().expect("not a number");
            info!("Bytes to read = {}", bytes_to_read);
            ...
            while bytes_to_read > 0 {
                ...
                let new_string = String::from_utf8(bytes_buffer[0..read_data_length].to_vec())
                    .expect("sent data is not UFT-8");
                info!("Read following bytes: {}", new_string);
                buffer.push_str(&new_string);
                bytes_to_read = bytes_to_read - read_data_length;
            }
        }
