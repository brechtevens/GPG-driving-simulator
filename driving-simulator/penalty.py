...
class PenaltyUpdater(object):
    ...
    def __init__(self, penalty_parameters):
        ...
        self._settings = penalty_parameters.settings
        self._penalty_parameter_values = cs.DM.ones(self._penalty_parameters.nb_all) *\
                                         self._settings.initial_penalty
        self._constraint_function = None
        self._penalty_update_function = None
        self._indices_to_increase = []
        self._max_penalty_value = self._settings.initial_penalty *\
                                  self._settings.penalty_update_factor ** self._settings.max_outer_iterations
    ...
    def update_index_set(self, p_current):
        if self._constraint_function is None:
            ...
            if self._settings.penalty_norm == 'norm_inf':
                violation_vector = cs.fabs(self._constraint_function(p_current)) > self._settings.penalty_eta
                ...
                self._indices_to_increase = cs.norm_2(self._constraint_function(p_current)) > self._settings.penalty_eta
        return
    ...
    def update_penalty_parameters(self):
        if self._settings.penalty_update_rule == 'individual':
            self._penalty_parameter_values[self._indices_to_increase] *= self._settings.penalty_update_factor
        ...
        if self._settings.penalty_update_rule == 'collective':
            self._penalty_parameter_values *= self._settings.penalty_update_factor
        return
    ...
    def shift_penalty_parameters(self):
        ...
        for k in range(N-1):
            self._penalty_parameter_values[k*M:(k+1)*M] = self._penalty_parameter_values[(k+1)*M:(k+2)*M]
        self._penalty_parameter_values[(N-1) * M:N * M] = cs.DM.ones(M) * self._settings.initial_penalty
        # self._penalty_parameter_values[:N * M] = cs.fmax(self._settings.initial_penalty,
        #                             self._penalty_parameter_values[:N * M]/(self._settings.penalty_update_factor**2))
