...
# Rectangle formulation
def rectangle_formulation_squared_distance_to_target():
...
def rectangle_formulation_inequality_constraints(car1, car2, epsilon=1e-2):
    """ Returns the 8 inequality constraints for the rectangle formulation
    ...
    """
    squared_dist = rectangle_formulation_squared_distance_to_target()

    @constraints.stageconstraints
    def h(x, u=None):
        constraint_list = []
        for corner in car1.corners(x[car1.id]):
            constraint_list.append(squared_dist(car2,x[car2.id],corner) - epsilon**2)
        for corner in car2.corners(x[car2.id]):
...
def affine_edges(target_vehicle, x_target):
    ...
    x_target : cs.DM
        the position of the target vehicle
    """
    target_corners = target_vehicle.corners(x_target)
...
def product_formulation_equality_constraints(car1, car2, epsilon=1e-5):
    """ Returns the 8 equality constraints for the product formulation
    ...
    epsilon : float, optional
        the minimal distance required between both vehicles, i.e. virtual enlargement
    """
    h = product_formulation_equality_constraint_of_target_vehicle(epsilon)

    @constraints.stageconstraints
    def g(x, u=None):
        constraint_list = []
        for corner in car1.corners(x[car1.id]):
            constraint_list.append(h(car2, x[car2.id], corner))
        for corner in car2.corners(x[car2.id]):
            constraint_list.append(h(car1, x[car1.id], corner))
...
def dual_formulation_constraints(car1, car2, epsilon=1e-5):
...
if __name__ == '__main__':
    ...
    car1 = car.GNEPOptimizerCar(dyn, [0., 0., math.pi / 2., 5.], color='red', id=0, T=20)
    car2 = car.GNEPOptimizerCar(dyn, [1., 3., math.pi / 2., 5.], color='yellow', id=1, T=20)
    angle = math.pi/2
    ...
    collision_function = rectangle_formulation_squared_distance_to_target()
    ...
    # tests for rectangle formulation
    rectangle_formulation_inequality_constraints = rectangle_formulation_inequality_constraints(car1, car2)
    test_formulation('rectangle formulation', rectangle_formulation_inequality_constraints, angle, 'inequality')
    ...
    test_car = car.GNEPOptimizerCar(dyn, [2., 0., 0., 5.], color='red', id=0, T=20)
    squared_distance = rectangle_formulation_squared_distance_to_target()
