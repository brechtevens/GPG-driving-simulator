...
class Logger:
    ...
    @property
    def location(self):
        return 'experiments/' + self.settings.name_experiment + '/'
        ...
        for key in self.history_keys:
            ...
            for vehicle in cars:
                if key in self.history_keys[:6]:
                    ...
                    if isinstance(vehicle, car.GPGOptimizerCar):
                        self.history[key][vehicle.id] = {}
                        for id in vehicle.get_current_belief():
                            self.history[key][vehicle.id][id] = []
        if len(cars) == 2:
            self.history['headway'] = []
            self.squared_distances = collision.rectangle_formulation_inequality_constraints(cars[0], cars[1])
    ...
    def log_data(self, cars):
        ...
        for vehicle in cars:
            ...
            if isinstance(vehicle, car.GPGOptimizerCar):
                r = vehicle.get_current_reward(vehicle.x, vehicle.u, x_dict)
                violation = 0
                g_list = vehicle.stage_g + vehicle.soft_stage_g + vehicle.player_stage_g[vehicle.id]
                h_list = vehicle.stage_h + vehicle.soft_stage_h + vehicle.player_stage_h[vehicle.id]
                for con in g_list:
                ...
                for id, params in vehicle.get_current_belief().items():
                    self.history['belief'][vehicle.id][id].append(params)
        if len(cars) == 2:
        ...
        if not self.settings.only_save_statistics:
            for key, data in self.history.items():
                if key == 'headway' or key == 'potential':
                    iter_data = np.vstack(
                        (range(1, self.settings.nb_iterations_experiment + 1), data)).transpose()
                    time_data = np.vstack(
                        ([i * self.Ts for i in range(self.settings.nb_iterations_experiment)], data)) \
                        .transpose()
                    ...
                    for id, dataline in data.items():
                        if key == 'belief':
                            for id_other, datalineline in dataline.items():
                                for i in range(len(datalineline[0])):
                                    belief_list = [item[i] for item in datalineline]
                            ...
                            iter_data = np.vstack(
                                (range(1, self.settings.nb_iterations_experiment + 1), dataline)).transpose()
            ...
            for id in self.history['x']:
                np.savetxt(additional_path + '/trajectory_' + str(id) + '.dat',
                           np.vstack((self.history['x'][id], self.history['y'][id])).transpose(), delimiter='\t',
                           fmt='%16.10f')
